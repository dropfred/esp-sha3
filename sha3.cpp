...
namespace
{
	...
	void keccak_round(std::uint64_t S[5][5], std::uint64_t RC)
	{
		std::uint64_t BCD[5][5];
		/* theta */
		for (std::size_t x = 0; x < 5; ++x)
		{
			BCD[0][x] = S[0][x] ^ S[1][x] ^ S[2][x] ^ S[3][x] ^ S[4][x];
		}
		for (std::size_t x = 0; x < 5; ++x)
		{
		...
		for (std::size_t x = 0; x < 5; ++x)
		{
			for (std::size_t y = 0; y < 5; ++y)
			{
				BCD[((2 * x) + (3 * y)) % 5][y] = keccak_rotl(S[y][x], keccak_rot[x][y]);
			}
		}
		/* chi */
		for (std::size_t x = 0; x < 5; ++x)
		{
			for (std::size_t y = 0; y < 5; ++y)
			{
				S[y][x] = BCD[y][x] ^ ((~BCD[y][(x + 1) % 5] & BCD[y][(x + 2) % 5]));
			}
		}
		/* iota */
		S[0][0] ^= RC;
	...
	void keccak_round(std::uint64_t S[5][5], std::uint64_t RC)
	{
		...
		for (std::size_t x = 0; x < 5; ++x)
		{
			BCD[x][0] = S[x][0] ^ S[x][1] ^ S[x][2] ^ S[x][3] ^ S[x][4];
		}
		for (std::size_t x = 0; x < 5; ++x)
		{
		...
		for (std::size_t x = 0; x < 5; ++x)
		{
			for (std::size_t y = 0; y < 5; ++y)
			{
				BCD[y][((2 * x) + (3 * y)) % 5] = keccak_rotl(S[x][y], keccak_rot[x][y]);
			}
		}
		/* chi */
		for (std::size_t x = 0; x < 5; ++x)
		{
			for (std::size_t y = 0; y < 5; ++y)
			{
				S[x][y] = BCD[x][y] ^ ((~BCD[(x + 1) % 5][y] & BCD[(x + 2) % 5][y]));
			}
		}
		/* iota */
		S[0][0] ^= RC;
	...
	void keccak_f(std::uint64_t S[5][5])
	{
#if (SHA3_TRANSPOSE == SHA3_TRANSPOSE_STATE)
		std::uint64_t ST[5][5];
		transpose(S, ST);
		...
		{
			keccak_round(state, keccak_rc[r]);
		}
#if (SHA3_TRANSPOSE == SHA3_TRANSPOSE_STATE)
		transpose(state, S);
#endif
